...
export function threadFromAddresses(addresses, domain) {
  ...
  for (const value of addresses) {
    ...
    const match = address
      .split('@')[0]
      .match(/^reply\+([0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})$/);
...
export function resendProvider({ key, from, replyDomain, fetcher = fetch }) {
  if (!key || !emailAddress(from) || !/^[a-z0-9-]+(?:\.[a-z0-9-]+)+$/i.test(replyDomain || ''))
    throw Error('email_not_configured');
  const request = async (path, init = {}) =>
    boundedJson(
      await fetcher(`https://api.resend.com/${path}`, {
        ...init,
  ...
  return {
    async send(job) {
      if (
        !emailAddress(job.recipient) ||
        /[\r\n]/.test(job.subject) ||
        typeof job.body !== 'string' ||
        job.body.length > 6000 ||
        !/^[0-9a-f-]{36}$/.test(job.thread_id)
      )
        throw Error('invalid_email_job');
      const result = await request('emails', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Idempotency-Key': `form-email/${job.id}` },
        body: JSON.stringify({
          from,
          to: [job.recipient],
          subject: job.subject,
          text: job.body,
          reply_to: `reply+${job.thread_id}@${replyDomain.toLowerCase()}`,
        }),
      });
      if (typeof result.id !== 'string' || result.id.length > 200)
        throw Error('missing_provider_id');
      return result.id;
    },
    async receive(id) {
      if (!/^[0-9a-f-]{36}$/i.test(id || '')) throw Error('invalid_received_id');
      return request(`emails/receiving/${id}`);
    },
  };
}
export async function dispatchEmails({ backend, provider, enabled }) {
  ...
  const jobs = await backend.rpc('claim_form_emails');
  let processed = 0;
  for (const job of jobs || []) {
    try {
      const allowed = await backend.rpc('form_email_send_allowed', {
        p_id: job.id,
        p_lease_id: job.lease_id,
      });
      if (!allowed) throw Error('staff_access_revoked');
      const id = await provider.send(job);
      await backend.rpc('finish_form_email', {
        p_id: job.id,
        p_lease_id: job.lease_id,
        p_provider_id: id,
      });
      ...
      const safe = /^[a-z_]+(?:_[0-9]{3})?$/.test(error.message)
        ? error.message
...
export async function receiveEmail({ raw, headers, secret, replyDomain, backend, provider, now }) {
  ...
  const thread = threadFromAddresses(
  ...
  let body =
    typeof email.text === 'string' && email.text.trim()
      ? email.text.trim()
      : '[HTML-only email. Read the original in the email provider inbox before accepting.]';
  ...
  await backend.rpc('receive_form_email', {
    p_provider_id: email.id,
    p_thread_id: thread,
    p_sender: (emailAddress(email.from) || 'Unknown sender').slice(0, 320),
    p_subject: String(email.subject || '(No subject)')
      .replace(/[\r\n]/g, ' ')
      .slice(0, 160),
    p_body: body,
  });
