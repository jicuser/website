...
const backend = {
  ...
  download(path, file) {
    return download(
      `${base}/storage/v1/object/sermon-recordings/${path.split('/').map(encodeURIComponent).join('/')}`,
      file,
      { headers },
    );
  },
  async upload(path, bytes) {
    const response = await fetch(`${base}/storage/v1/object/sermon-recordings/${path}`, {
      method: 'POST',
...
for (const job of jobs || []) {
  ...
  try {
    ...
    const safeError = /^[a-z_]+(?:_[0-9]{3})?$/.test(error.message)
      ? error.message
      : 'processing_failed';
