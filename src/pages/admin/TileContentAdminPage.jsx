...
const DEFAULT_TILES = [
  ...
  },
  {
    ...
    text: 'Building for a stronger future.',
    image: '',
  },
...
async function uploadTileImage(file, key) {
  ...
  const path = `home-tiles/${key}-${crypto.randomUUID()}.${ext}`;
...
export default function TileContentAdminPage() {
      ...
      .then(({ data }) => {
        ...
        try {
          ...
          if (Array.isArray(saved))
            setTiles(
              DEFAULT_TILES.map((base) => ({
                ...base,
                ...(saved.find((x) => x.key === base.key) || {}),
              })),
            );
        } catch {}
      });
  ...
  const update = (index, patch) =>
    setTiles((rows) => rows.map((row, i) => (i === index ? { ...row, ...patch } : row)));
  const chooseImage = async (index, file) => {
    try {
      setBusy(true);
      setError('');
      setMessage('Uploading image…');
      const url = await uploadTileImage(file, tiles[index].key);
      if (url) update(index, { image: url });
      setMessage('Image uploaded. Press Save changes to publish it.');
    } catch (e) {
  ...
  const save = async () => {
    try {
      ...
      setMessage('Home tiles updated successfully.');
    } catch (e) {
      setError(e.message || String(e));
    } finally {
      setBusy(false);
  ...
  return (
    <div className="min-h-screen bg-slate-950 px-3 py-5 text-white sm:px-4 sm:py-8">
      <div className="mx-auto max-w-5xl">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <button
            onClick={save}
            disabled={busy}
            className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-amber-400 px-4 py-3 font-semibold text-slate-950 disabled:opacity-50 sm:w-auto"
          >
            <Save size={17} />
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {tiles.map((tile, index) => (
            <section
              key={tile.key}
              className="rounded-2xl border border-white/10 bg-white/[0.06] p-4 backdrop-blur-2xl sm:p-5"
            >
              <div className="mb-4 text-xs font-semibold uppercase tracking-[.16em] text-amber-300">
                Fixed tile: {tile.key}
              </div>
              <label className="mb-3 block text-sm font-medium">
                Heading
                <input
                  className="mt-1.5 w-full rounded-xl border border-white/10 bg-black/20 px-3 py-3 text-white outline-none focus:border-amber-300/50"
                  value={tile.title}
              ...
              {tile.image && (
                <img src={tile.image} alt="" className="mb-3 h-36 w-full rounded-xl object-cover" />
              )}
              <div className="grid gap-2 sm:grid-cols-[1fr_auto_auto]">
                <input
                  className="min-w-0 rounded-xl border border-white/10 bg-black/20 px-3 py-3 text-sm text-white"
                  placeholder="Optional image URL"
                  value={tile.image || ''}
                  onChange={(e) => update(index, { image: e.target.value })}
                />
                <label className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/10 px-3 py-3 text-sm">
                  <ImagePlus size={16} />
                  Upload
                  <input
                    type="file"
                    accept={IMAGE_ACCEPT}
                    className="hidden"
                    onChange={(e) => chooseImage(index, e.target.files?.[0])}
                  />
                </label>
                <button
                  type="button"
