    ...
    <div className="extension-hub-page">
      <div className="extension-hub-heading">
        ...
        <div className="extension-hub-controls" aria-label="Scroll project areas">
          <button type="button" onClick={() => scrollTiles(-1)} aria-label="Previous project area"><ArrowLeft size={18}/></button>
          <button type="button" onClick={() => scrollTiles(1)} aria-label="Next project area"><ArrowRight size={18}/></button>
        </div>
      </div>

      <div className="extension-hub-rail" ref={railRef}>
        {extensionAreas.map(({ title, text, to, icon: Icon }, index) => (
          <Link key={to} to={to} className={index === 0 ? 'extension-hub-tile is-featured' : 'extension-hub-tile'}>
            <span className="extension-hub-icon"><Icon size={24}/></span>
            <div>
              <h2>{title}</h2>
              <p>{text}</p>
            </div>
            <span className="extension-hub-open">Open <ArrowRight size={16}/></span>
          </Link>
        ))}
      </div>
    </div>
