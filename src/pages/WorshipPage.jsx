        ...
        <label>
          ...
          <select value={surah} onChange={event => setSurah(Number(event.target.value))}>
            {Array.from({ length: 114 }, (_, index) => index + 1).map(number => (
              <option key={number} value={number}>Surah {number}</option>
            ))}
          </select>
        </label>
    ...
    <div className="worship-page">
      {pathname === '/worship' && (
        <div className="worship-hub-grid">
          <Link to="/worship/quran"><BookOpen/><h2>Read Qur’an</h2><p>Read Arabic with English meaning.</p></Link>
          <Link to="/worship/dalail-al-khayrat"><HeartHandshake/><h2>Dala’il al-Khayrat</h2><p>Salawat and daily reading.</p></Link>
          <Link to="/worship/daily-duas"><MoonStar/><h2>Daily Du‘as</h2><p>Morning, evening and everyday adhkar.</p></Link>
          <Link to="/worship/prayer-guide"><MoonStar/><h2>Prayer Guide</h2><p>A compact Salah refresher.</p></Link>
          <Link to="/worship/janazah-guide"><HeartHandshake/><h2>Janazah Guide</h2><p>Know the basic funeral prayer steps.</p></Link>
          <Link to="/worship/daily-salah"><MoonStar/><h2>Daily Salah</h2><p>Today’s JIC prayer times.</p></Link>
        </div>
      )}
      {isQuran && <QuranReader/>}
