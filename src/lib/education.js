...
export function validSessions(sessions) {
  return sessions === undefined || (Array.isArray(sessions) && sessions.length <= 21 && sessions.every((s) =>
    Number.isInteger(s.day) && s.day >= 1 && s.day <= 7 &&
...
export function isAdultProgramme(poster) {
  return poster.kind !== 'announcement' && poster.groups?.includes('education') &&
    ['adult', 'all'].includes(poster.audience);
}
export function weeklySessions(posters) {
  return posters.filter(isAdultProgramme).flatMap((poster) => (poster.sessions || []).map((session) => ({
    ...session, id: poster.id, programme: poster.title, image: poster.image, to: poster.to,
