...
export const formStatus = (form) =>
  !form?.published_version ? 'Draft' : form.enabled ? 'Live' : 'Closed';
...
export function pageDraft(poster) {
  return {
    slug: poster?.id || pageSlug(poster?.title),
    title: poster?.title || '',
    body: poster?.detail || '',
    image_url: poster?.image || '',
    schedule: poster?.schedule || '',
    source_poster_id: poster?.id || null,
    placement: '/education',
    kind: 'page',
    registration: 'none',
    form_id: null,
...
export function pageProblem(page) {
  ...
  if (!/^[a-z0-9][a-z0-9-]{1,78}[a-z0-9]$/.test(page.slug))
    return 'Use a 3–80 character address with lowercase letters, numbers and hyphens.';
  if (!PAGE_PLACEMENTS.some(([path]) => path === page.placement))
    return 'Choose where this page appears.';
  if (!PAGE_KINDS.some(([kind]) => kind === page.kind)) return 'Choose the page type.';
  if (!REGISTRATION_TYPES.some(([kind]) => kind === page.registration))
    return 'Choose whether registration is needed.';
  if ((page.body || '').length > 12000 || (page.schedule || '').length > 400)
    return 'Shorten the page text.';
  if (page.image_url) {
    try {
      if (
        !(page.image_url.startsWith('/') || page.image_url.startsWith('https://')) ||
        /[\s\\]/.test(page.image_url) ||
        page.image_url.length > 2000
      )
        throw new Error();
      ...
      if (
        ...
        url.username ||
        url.password ||
        page.image_url.startsWith('//')
      )
        throw new Error();
    } catch {
      return 'Use an HTTPS picture address or an existing site image.';
    }
  }
  return '';
}
...
export function formAdminUrl(id, view = 'overview') {
  return `/admin?${new URLSearchParams({ section: 'forms', form: id, view })}`;
}
