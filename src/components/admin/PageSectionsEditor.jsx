...
export default function PageSectionsEditor({pagePath}){
  ...
  const add=async e=>{e.preventDefault();setBusy(true);setMsg('');try{const section_key=(form.section_key||form.title||`section-${Date.now()}`).toLowerCase().trim().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');const{error}=await supabase.from('page_sections').insert({...form,page_path:pagePath,section_key}).select('id').single();if(error)throw error;setForm(blank);window.dispatchEvent(new Event('jic-content-updated'));setMsg('Section published.');await load()}catch(x){setMsg(x.message)}finally{setBusy(false)}};
  const patch=async(id,patch)=>{setBusy(true);setMsg('');try{const{error}=await supabase.from('page_sections').update(patch).eq('id',id).select('id').single();if(error)throw error;window.dispatchEvent(new Event('jic-content-updated'));setMsg('Section updated.');await load()}catch(x){setMsg(x.message)}finally{setBusy(false)}};
