...
export default function PageSectionsEditor({ pagePath }) {
  ...
  const load = useCallback(async () => {
    const { data, error } = await supabase.from('page_sections').select('*').eq('page_path', pagePath).order('sort_order');
  ...
  const add = async event => {
    ...
    try {
      ...
      const { error } = await supabase.from('page_sections').insert({ ...form, page_path: pagePath, section_key: sectionKey }).select('id').single();
      if (error) throw error;
      setForm(blank);
  ...
  const saveRow = useCallback(async (id, draft, pendingBackground, pendingGallery) => {
    setBusy(true);
    setMsg('');
    try {
      ...
      if (pendingBackground) backgroundImageUrl = await uploadImage(pendingBackground, `sections/${folder}/backgrounds`);
      for (const file of pendingGallery || []) imageUrls.push(await uploadImage(file, `sections/${folder}/gallery`));
      const payload = {
        title: draft.title || '',
        body: draft.body || '',
      ...
      const { error } = await supabase.from('page_sections').update(payload).eq('id', id).select('id').single();
      if (error) throw error;
      window.dispatchEvent(new Event('jic-content-updated'));
  ...
  const del = async id => {
    ...
    setBusy(true);
    const { error } = await supabase.from('page_sections').delete().eq('id', id).select('id').single();
  ...
  return <section className="admin-panel">
    <div className="admin-heading"><div><h3>Page sections</h3></div></div>
    {msg && <div className="admin-hint">{msg}</div>}
    <form onSubmit={add} className="admin-section-create">
      <label>Section name<input value={form.title} onChange={event => setForm({ ...form, title: event.target.value })}/></label>
      <label>Information<textarea rows="3" value={form.body} onChange={event => setForm({ ...form, body: event.target.value })}/></label>
      <label>Order<input type="number" value={form.sort_order} onChange={event => setForm({ ...form, sort_order: Number(event.target.value) })}/></label>
      <button disabled={busy || !form.title.trim()} className="admin-button primary"><Plus size={16}/>Add section</button>
    </form>
