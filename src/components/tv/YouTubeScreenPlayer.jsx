...
export default function YouTubeScreenPlayer({ videoId, title, muted, onUnavailable }) {
  ...
  useEffect(() => {
    ...
    loadPlayerApi()
          ...
          events: {
            ...
            onStateChange: (event) => {
              ...
              if (event.data === YT.PlayerState.ENDED)
                unavailable.current('The livestream has ended.');
            },
            onError: () => {
              if (active) unavailable.current('The livestream is unavailable. Showing posters.');
            },
            onAutoplayBlocked: () => {
      ...
      .catch(() => {
        if (active) unavailable.current('The livestream could not load. Showing posters.');
      });
    return () => {
      active = false;
      player.current = null;
  ...
  return (
    <div className="jic-tv-player">
      <div className="jic-tv-player-host" ref={host} />
      {blocked && (
        <button
          className="jic-tv-start-video"
          type="button"
