...
export default function ProgrammePosters() {
  ...
  return <>
    <SwipeRail title={group === 'home' ? 'What’s on & learning' : 'Classes & gatherings'} className="jic-programmes">
      {posters.map(item => <article className="jic-programme-card" key={item.id}>
        <button type="button" className="jic-poster-button" onClick={() => setSelected(item)} aria-label={`View ${item.title} poster`}><img src={item.image} alt={item.alt} loading="lazy" width="1224" height="1730"/><span><Expand size={16}/> View poster</span></button>
