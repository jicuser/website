...
export default function DailyReminder() {
  ...
  useEffect(() => {
    let scrollTimer, scrollIntentUntil = 0;
    const isEditing = target => target instanceof Element && target.closest('input,textarea,select,[contenteditable="true"],.admin-console,.jic-daily-reminder');
    ...
    ['wheel','touchstart','touchmove','pointerdown','keydown'].forEach(type => document.addEventListener(type, onIntent, { capture:true, passive:true }));
    return () => {
      window.clearTimeout(scrollTimer);
      document.removeEventListener('click', onClick, true);
      document.removeEventListener('scroll', onScroll, true);
      ['wheel','touchstart','touchmove','pointerdown','keydown'].forEach(type => document.removeEventListener(type, onIntent, true));
    };
  }, [advance]);
  let reminders = DEFAULT_REMINDERS, notices = [];
  try { const saved = JSON.parse(getContent('header_reminders', '[]')); const valid = saved.filter(item => typeof item?.text === 'string' && item.text.trim() && typeof item?.type === 'string' && (item.source == null || typeof item.source === 'string')); if (valid.length) reminders = [...valid, ...DEFAULT_REMINDERS.filter(item => !valid.some(custom => custom.text.trim() === item.text))]; } catch { /* Keep curated defaults. */ }
  ...
  return <aside className="jic-daily-reminder" aria-label="Daily reminder">
    <button type="button" aria-label="Show another daily reminder" title="Changes after two taps or scroll gestures. Tap here for the next reminder." onClick={advance}><div><b>{reminder.type}</b><span>{date}</span><small>{reminder.source}</small></div><p>{reminder.text}</p></button>
