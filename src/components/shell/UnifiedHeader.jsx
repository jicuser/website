...
import { Heart, MapPin, Menu, Moon, Pause, Play, Phone, Sun, X, LogIn, Sparkles } from 'lucide-react';
...
export default function UnifiedHeader() {
  ...
  const [reminderIndex, setReminderIndex] = useState(
    () => Math.floor(Date.now() / 86400000) % REMINDERS.length
  );
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      if (nearTop && !scrollStateRef.current) {
        setReminderIndex(index => (index + 1) % REMINDERS.length);
      }
      ...
      <div className="jic-unified-inner">
        <div className="jic-unified-info jic-glass">
          <div className="jic-unified-address">
            <span><MapPin size={15}/>{SITE.address.short}</span>
        ...
        <button
          ...
          onClick={() => setReminderIndex(index => (index + 1) % REMINDERS.length)}
            ...
            <div className="jic-unified-menu-group" key={path}>
              <div className="jic-unified-menu-row">
                ...
                {children.length > 0 && (
                  <button
                    ...
                    aria-label={`${mobileGroup === name ? 'Collapse' : 'Expand'} ${name}`}
                  >
                    {mobileGroup === name ? '−' : '+'}
                  </button>
                )}
              ...
              {children.length > 0 && mobileGroup === name && (
                <div className="jic-unified-menu-children">
                  {children.filter(child => name === 'Education' || child.path !== path).map(child => (
