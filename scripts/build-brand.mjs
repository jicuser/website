...
const wrap = (name, width, height, body) => `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-label="Jamatia Islamic Centre"><title>Jamatia Islamic Centre — ${name}</title>${body}</svg>\n`;
const variants = ['horizontal', 'centred', 'compact', 'pillars', 'arch', 'minaret', 'minaret-compact'];
const artwork = Object.fromEntries(variants.map(name => [name, readFileSync(`${source}/${name}.svg`, 'utf8')]));
artwork.wordmark = wrap('Lettering', 680, 265, lettering.replace(' transform="translate(299,230)"', ''));
artwork.entrance = wrap('Entrance symbol', 926, 1064, `${definitions}<g transform="translate(-180 -62)">${entranceArt}</g>`);
...
for (const [name, original] of Object.entries(artwork)) {
  for (const theme of ['light', 'dark']) {
    ...
    await sharp(Buffer.from(svg)).resize({ width: 1200, height: 1200, fit: 'inside' }).png().toFile(`${path}.png`);
  }
}

writeFileSync(`${destination}/jic-variations.svg`, readFileSync(`${source}/variations.svg`, 'utf8'));

// Favicon and home-screen artwork use the same entrance, without tiny lettering.
const tile = (scale, x, y) => wrap('App icon', 512, 512,
...
writeFileSync('public/favicon.svg', tile(.447, 49, 18));
mkdirSync('public/icons', { recursive: true });
await sharp(Buffer.from(tile(.32, 108, 84))).png().toFile('public/icons/jic-icon-maskable-512.png');
