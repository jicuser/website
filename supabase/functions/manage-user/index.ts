...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const actorName = String(user.user_metadata?.audit_name || profile.display_name || 'Staff')
      .trim()
      .slice(0, 120);
    ...
    if (body.action === 'invite') {
      ...
      if (data.user) {
        const { error: roleError } = await adminClient
          .from('profiles')
          .update({ role: body.role, is_active: true })
          .eq('id', data.user.id);
        if (roleError) throw roleError;
        await audit('INVITE', data.user.id, null, {
    ...
    if (body.action === 'set_role') {
      if (!body.user_id || !allowedRoles.includes(body.role))
        throw new Error('Invalid user or role');
      if (body.user_id === user.id && body.role !== 'super_admin')
    ...
    if (body.action === 'set_active') {
      ...
      if (body.user_id === user.id && body.is_active === false)
