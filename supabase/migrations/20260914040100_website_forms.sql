-- Website forms reuse the established form API without app or remote-worker tables.
begin;
...
create table public.user_notifications (
 ...
 kind text not null check(kind in('task','form')), entity_id uuid not null,
...
create or replace function private.read_task(task uuid,account uuid default auth.uid()) returns boolean language sql stable security definer set search_path='' as $$
 ...
 where t.id=task and (t.form_id is not null or p.is_owner or t.assigned_to=account or t.created_by=account)
...
alter table public.user_notifications add constraint user_notifications_kind_check check(kind in('task','form'));
...
-- Task and notification reads follow current account and form membership.
alter table public.work_tasks enable row level security;
alter table public.user_notifications enable row level security;
revoke all on public.work_tasks,public.user_notifications from public,anon,authenticated;
grant all on public.work_tasks,public.user_notifications to service_role;
grant select on public.work_tasks,public.user_notifications to authenticated;
...
create policy notification_read on public.user_notifications for select to authenticated using(user_id=auth.uid() and private.can_receive_notification(user_id,kind,entity_id));
create policy notification_update on public.user_notifications for update to authenticated using(user_id=auth.uid() and private.can_receive_notification(user_id,kind,entity_id)) with check(user_id=auth.uid() and private.can_receive_notification(user_id,kind,entity_id));
create function private.website_task_stamp() returns trigger language plpgsql set search_path='' as $$
 begin new.updated_at=clock_timestamp(); return new; end; $$;
create trigger task_stamp before update on public.work_tasks for each row execute function private.website_task_stamp();
create function private.website_task_notify() returns trigger language plpgsql security definer set search_path='' as $$
 begin
 if tg_op='INSERT' or new.assigned_to is distinct from old.assigned_to then
 insert into public.user_notifications(user_id,kind,entity_id) values(new.assigned_to,'task',new.id);
 end if; return new; end; $$;
create trigger task_notify after insert or update of assigned_to on public.work_tasks for each row execute function private.website_task_notify();
create function public.update_form_task(p_task_id uuid,p_status text,p_assigned_to uuid) returns void language plpgsql security definer set search_path='' as $$
 declare task public.work_tasks;
 begin
 select * into task from public.work_tasks where id=p_task_id for update;
 if task.id is null or not private.staff_custom_submission(task.form_id) then raise exception 'Form staff access required'; end if;
 if not private.staff_custom_submission(task.form_id,p_assigned_to) then raise exception 'Choose an active member of this form'; end if;
 if p_status not in('open','in_progress','waiting','done') then raise exception 'Choose a valid action status'; end if;
 update public.work_tasks set status=p_status,assigned_to=p_assigned_to where id=task.id;
 end; $$;
-- This computed marker only opens the admin Forms area. It is not an assignable global permission.
create or replace function public.get_my_profile() returns table(id uuid,display_name text,is_active boolean,is_owner boolean,permissions text[],staff_kinds text[])
 language sql stable security definer set search_path='' as $$
 select p.id,p.display_name,p.is_active,p.is_owner,
 p.permissions || case when exists(select 1 from public.custom_form_staff s where s.user_id=p.id) then array['forms_assigned'] else array[]::text[] end,p.staff_kinds
 from public.profiles p where p.id=auth.uid(); $$;
-- Compact server summaries avoid one count request per visible form tile.
create function public.admin_form_catalogue() returns jsonb language plpgsql stable security definer set search_path='' as $$
 begin
 if not private.active_account(auth.uid()) then raise exception 'Active account required'; end if;
 return coalesce((select jsonb_agg(to_jsonb(f)||jsonb_build_object(
 'can_manage',private.manage_custom_form(f.id),
 'responsible',coalesce((select jsonb_agg(jsonb_build_object('id',p.id,'display_name',p.display_name,'is_active',p.is_active)) from public.custom_form_staff s join public.profiles p on p.id=s.user_id where s.form_id=f.id and s.role='responsible'),'[]'::jsonb),
 'response_count',(select count(*) from public.form_submissions r where r.custom_form_id=f.id),
 'new_count',(select count(*) from public.form_submissions r where r.custom_form_id=f.id and r.status='new'),
 'open_actions',(select count(*) from public.work_tasks t join public.form_submissions r on r.id=t.form_id where r.custom_form_id=f.id and t.status<>'done')
 ) order by f.updated_at desc) from public.custom_forms f where private.staff_custom_form(f.id)),'[]'::jsonb);
 end; $$;
revoke all on function public.get_my_profile(),public.update_form_task(uuid,text,uuid),public.admin_form_catalogue() from public,anon;
grant execute on function public.get_my_profile(),public.update_form_task(uuid,text,uuid),public.admin_form_catalogue() to authenticated,service_role;
revoke all on function private.active_account(uuid),private.workspace_owner(),private.account_permission(uuid,text),private.read_task(uuid,uuid),private.can_receive_notification(uuid,text,uuid),private.website_task_stamp(),private.website_task_notify() from public,anon,authenticated;
grant execute on function private.active_account(uuid),private.workspace_owner(),private.account_permission(uuid,text),private.read_task(uuid,uuid),private.can_receive_notification(uuid,text,uuid) to authenticated,service_role;
commit;
