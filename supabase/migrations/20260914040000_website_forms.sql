-- Website forms, private submissions and in-admin actions.
begin;
...
create function private.can_receive_notification(account uuid,kind text,entity uuid) returns boolean language sql stable security definer set search_path='' as $$ select kind='task' and private.read_task(entity,account); $$;
...
create function private.publish_custom_form(p_form_id uuid,p_expected_updated_at timestamptz default null) returns integer language plpgsql security definer set search_path='' as $$
...
create function public.publish_custom_form(p_form_id uuid,p_expected_updated_at timestamptz default null) returns integer language sql security invoker set search_path='' as $$ select private.publish_custom_form(p_form_id,p_expected_updated_at); $$;
create function private.get_public_form(p_slug text) returns jsonb language sql stable security definer set search_path='' as $$
 ...
 from public.custom_forms f join public.custom_form_versions v on v.form_id=f.id and v.version=f.published_version where f.slug=p_slug and f.enabled and exists(select 1 from public.custom_form_staff s where s.form_id=f.id and s.role='responsible' and private.active_account(s.user_id)); $$;
...
create function private.list_public_forms() returns table(id uuid,slug text,title text,description text,version integer) language sql stable security definer set search_path='' as $$
 select f.id,f.slug,v.title,v.description,v.version from public.custom_forms f join public.custom_form_versions v on v.form_id=f.id and v.version=f.published_version where f.enabled and exists(select 1 from public.custom_form_staff s where s.form_id=f.id and s.role='responsible' and private.active_account(s.user_id)) order by v.title limit 200; $$;
