...
test('fee ledger enforces source access, integer balances, immutable receipts and exact provider reconciliation',async(t)=>{
 ...
 for(const name of (await readdir(new URL('../supabase/migrations/',import.meta.url))).filter((name)=>name.startsWith('20260913')).sort())await db.exec(await readFile(new URL(`../supabase/migrations/${name}`,import.meta.url),'utf8'));
 ...
 await t.test('all additive migrations compose: custom form reply, fee and optional email queue share one response',async()=>{
 const definition=await as(owner,()=>rpc('save_custom_form',[JSON.stringify({slug:'integrated-form',title:'Integrated form',schema:{fields:[{id:'name',label:'Name',type:'text',required:true}]},responsible_ids:[teacher]})]));
 await as(owner,()=>rpc('publish_custom_form',[definition]));
 const response=await as(null,()=>rpc('submit_custom_form',['integrated-form',1,JSON.stringify({name:'Parent'}),'a'.repeat(64),id(70),parent,'[]']),'service_role');
 await as(teacher,()=>rpc('reply_custom_form',[response,'Your place is confirmed',false]));
 const charge=await as(teacher,()=>rpc('create_fee_request',['Class materials',1500,'GBP',response,null,null,null,id(71)]));
 await as(parent,async()=>{const rows=await db.query('select * from form_replies where submission_id=$1',[response]);assert.equal(rows.rows[0].author_kind,'account');assert.equal((await rpc('list_fee_requests',[response])).rows[0].id,charge);});
 await as(null,()=>rpc('set_form_email_ready',[true]),'service_role');
 await as(teacher,()=>rpc('queue_form_email',[response,'parent@example.org','Class materials','Please review the fee in your account',id(72),true]));
 assert.equal((await db.query('select * from form_email_outbox where submission_id=$1',[response])).rows.length,1);
 await as(other,async()=>{assert.equal((await db.query('select * from form_email_outbox')).rows.length,0);assert.equal((await rpc('list_fee_requests',[response])).total,0);});
 });
 await db.close();
});
