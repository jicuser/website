...
Deno.test(
  'custom forms handler verifies identities and bounds request, upload and export access',
  async (t) => {
    const values: Record<string, string> = {
      SUPABASE_URL: 'https://forms.invalid',
    ...
    globalThis.fetch = (async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (path === '/auth/v1/user') {
        ...
        return token === 'Bearer valid-user-token'
          ? json({ id: owner, email: 'member@example.org' })
      ...
      if (path.endsWith('/get_custom_form_upload'))
        return json({
          id: upload,
          ready: false,
          object_key: 'safe/random',
          mime_type: 'image/png',
          size_bytes: 24,
        });
      if (path === '/storage/v1/object/authenticated/form-attachments/safe/random')
        return new Response(
          invalidFile ? new TextEncoder().encode('<html>not an image</html>') : new Uint8Array(24),
        );
      if (path === '/storage/v1/object/form-attachments') return json([]);
      if (path.endsWith('/custom_form_export'))
        return json({ message: 'Form staff access required', code: 'P0001' }, 400);
      throw new Error(`Unmocked network request: ${path}`);
    }) as typeof fetch;
    const post = (body: unknown, token = 'test-anon-key') =>
      handler(
        new Request('https://edge.invalid/custom-forms', {
          method: 'POST',
    ...
    try {
      ...
      await t.step(
        'derives submitter from verified JWT and ignores client supplied identity',
        async () => {
          calls.length = 0;
          const response = await post(
            {
              ...
              slug: 'help-form',
              version: 1,
              answers: { name: 'A member' },
              idempotency_key: submission,
              p_user_id: submission,
              user_id: submission,
            },
            'valid-user-token',
          );
          ...
          assert(
            typeof call?.body.p_source_key === 'string' && call.body.p_source_key.length === 64,
          );
      ...
      await t.step('does not hand unrelated users an attachment URL or export bytes', async () => {
        const download = await post(
          { action: 'download', attachment_id: upload },
          'valid-user-token',
        );
        assert(download.status === 403);
        const exported = await post(
          { action: 'export', form_id: owner, format: 'zip' },
          'valid-user-token',
        );
        assert(exported.status === 403);
      });
      await t.step(
        'rejects file bytes that do not match the declared image signature',
        async () => {
          invalidFile = true;
          calls.length = 0;
          const response = await post({
