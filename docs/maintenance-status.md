...
- No DNS, registrar, mail DNS, hosting or Cloudflare configuration changes in this delivery.
...
| --- | --- | --- | --- |
| WEB-01 | Mobile poster picker | Titles below images; editor brought into view without opening the keyboard. Full-app browser checks pass at 320, 390 and 1280px. Website release published. | Confirm editing on the physical phone with the live account. |
| WEB-02 | Admin section navigation | One Go to selector in the sticky mobile toolbar; desktop sidebar retained. Full-app navigation and publishing checks pass; toolbar class verified in the live bundle. | Confirm Notices and poster editing on the physical phone. |
| WEB-03 | Owner-only deletion | Typed confirmation; disabled non-owner accounts only; self/owners protected; source and audit checks. `manage-user` version 5 deployed with JWT verification retained; controls present in the live bundle. | Real signed-in operator acceptance; no production account deleted by tests. Historical records/uploads can intentionally block deletion. |
...
| WEB-06 | Browser connection warning | HTTPS login/setup rendered during read-only checks; HTTP login resolved to HTTPS. No warning observed in the latest browser check; no certificate bypass used. | The original phone warning has not been reproduced. Its exact cause is still unconfirmed; these checks are not a complete security audit. |
...
Reviewed release plus scope record: `c8cfcc736a665b3cb92e26dc965d84f45e9aa063`.
Successful GitHub Actions runs: `34795929942` and `34796339248`.
...
- `main` was fast-forwarded to `c8cfcc7` after validation; GitHub confirmed the ref. No concurrent changes were overwritten.
- A subsequent read-only production browser check rendered `/admin/login` and fetched `/assets/AdminPage-DENwNEw2.js` with HTTP 200. That bundle contains the new deletion controls and `admin-mobile-section w-full`. No login, form submission or certificate bypass was performed. This confirms the changed frontend bundle is served; it is not an authenticated live-user workflow test.
- Real email acceptance, live account deletion and physical TV playback remain distinct acceptance checks. Browser/Node substitutes do not prove these outcomes.

Rollback reference: `71e323290062514b6f6777c9df601875bdad2107`. Revert focused
commits rather than erasing history or loosening database constraints. Backend
rollback uses the previous reviewed function source (version 4), not a schema rollback.
The `maintenance/admin-reliability` branch remains available for release comparison.

See `auth-email-configuration.md` for the hosted email checklist and conservative
account-deletion dependency policy. Update this tracker instead of creating another
competing maintenance document.
